...
LC_ALL=C /usr/bin/bsdtar -tvf "$fixtures/small-archive.zip" >/dev/null || fail "selected zip archive is not listable by the external fixture oracle"
LC_ALL=C /usr/bin/bsdtar -tvf "$fixtures/small-archive.tar" >/dev/null || fail "selected tar archive is not listable by the external fixture oracle"
